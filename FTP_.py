...
#---------------------------------------------log----------------------------------------------------------------
def create_logging(filepath = 'watcher.log',mode = 'a' ):
    ...
    def sync(self, source, destination, tree = False):
        ...
        try:
            if not os.path.exists(destination):
                ...
                self.traverse_DownloadFile(tree)
                out_("完成了 {} 个文件".format(len(os.listdir(destination))))
            else:
                print "{} has been there".format(destination)
                out_("{} has been there".format(destination))
        except OSError:     
